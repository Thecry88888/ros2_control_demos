...
int main(int argc, char ** argv)
{
  ...
  auto pub = node->create_publisher<trajectory_msgs::msg::JointTrajectory>(
    // "/lrmate_200id_controller/joint_trajectory", 10);
  ...
  robot_tree.getChain("base_link", "tool0", chain); // chain 包含了從基座到末端執行器的所有可動關節
  ...
  for (int i = 0; i < trajectory_len; i++)
  {
    ...
    twist.vel.x(0.3 * cos(2 * M_PI * t));
    twist.vel.z(-0.3 * sin(2 * M_PI * t));
